...
- **pass 5** (`pass5`): found that every previous in-game sheet was taken facing the reception counter, so the wall-clip avoidance had been tilting and pulling the tools in every shot; the shot tools now turn to a heading with three clear cells ahead. Clean verdict: spray gun firing and brush aiming read as real hands on tools; the nailer aims with both hands; the launcher still framed its breech cap square-on.
- **pass 6** (`pass6`): the launcher is a shoulder weapon, so its aim pose moved beside the head (x 0.14, y −0.2, z −0.76, slight yaw); the ring sight now sits on the rail in view and the fire pose kicks the tube up.
Remaining at-a-glance faults after pass 6: at hip most tools show only their nose (hands below the frame); the tools themselves (blob-lofted bodies, flat plastics) are now the weakest element next to the hands; the leg's studio framing is still off.
...
## Round 15 — richer weapons, click-between-versions   (2026-09-04, `docs/modern/GOAL_LOOP_11.md`)

- **T1 brush** (`docs/evidence/T1`): rebuilt — varnished turned handle with grain, hang hole, varnish chips and old splatter; crimped brass ferrule with rivets both faces and paint smears; the bristles first came out as a rake of separate strands (verdict on `studio1`), so they became one striated slab with tip fuzz; a glossy blue paint shell wraps the last third with a sag and four drips with beads. Verdict on `studio2`/`crops2`: reads as a loaded sash brush.
- **T2 nailer**: dimpled rubber overmould on the grip, warning label, bright wear on the nose casting edges, LED glow disc. **T3 spray gun**: dried paint runs down the cup from the rim, smears on the flank, hose strain relief. **T4 launcher**: paint runs from the bell lip, a wet ring inside it, a tube decal (dents tried and removed — they read as blobs). **T5 leg**: fraying tape edge, splinters where the plate tore off, varnish chips. Verdicts on `studio3`/`crops3`.
- **T6 versions**: a sixth main-menu entry VERSIONS opens a clickable list of every generation (3 · Modern "you are here", 2.1, 2.0 "PLAY", 1 "ABOUT"); bundled generations open in an **in-page player** (iframe overlay with a BACK TO MODERN bar, OPEN IN A NEW TAB, and a pulsing LOADING label until the build's load event), so switching versions never navigates away; the Options row still works and uses the same player. Found and fixed a pre-existing bug: `gen-card` was never in the screen list, so the Gen 1 card could not be shown since round 4. Verified (`docs/evidence/T6`): menu with six items, the Versions screen, the Gen 1 card, Gen 2.0 and Gen 2.1 booting inside the player (2.1 takes ~14 s under the software renderer), back returns to the Versions screen with the menu music. Note: while the iframe has focus, Esc goes to the inner game; the bar button is the way back.
- **Gate**: frozen guard OK; levels valid; build 14.75 MB; collision hashes identical to the classic; smoke green on the built file (after dropping `fullscreen; autoplay` from the player iframe's allow list — the browser logs permission-policy warnings for them on file:// and the smoke counts those as errors). Committed; `main` fast-forwarded; tagged `v3.9-modern`. Push still owed.

---

## Round 16 — 4.0: publish   (2026-09-04)

- README rewritten around the modern build: the cover art (`tableboxart2.png`, the same image the menu shows) at the top; fresh captures from the built file (`tools/readme_shots.mjs` → `docs/readme/modern-*.jpg`: menu, Versions, all six floors with the floating tools, the bench at low health, the crawl, the player running generation 2.1, studio views of the brush, spray gun and launcher); a "Four generations, one file" section explaining the Versions screen and what "the same game" means; controls including aim, quick melee, inspect and sprint toggle; the build commands as they are now (`dev` = modern on 5174, `preview` on 4174, `dev:classic` on 5173); a documentation table; and a "The classic generation" section that keeps the 2.1 captures and its own design notes.
- `docs/modern/INDEX.md`: start-here index of every goal loop, the log, the evidence, the tools and the harness.
- `CHANGELOG.md` 4.0 entry; `package.json` version 4.0.0.
- `main` fast-forwarded to `modern-preview`; tagged `v4.0-modern`. **Push to GitHub blocked:** the stored `gh` token is invalid and there is no SSH key on this machine (`git push` is refused with "Invalid username or token"). After `gh auth login -h github.com`: `git push origin main modern-preview --tags`.
